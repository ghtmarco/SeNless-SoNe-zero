...
class _AddEditVideoPageState extends State<AddEditVideoPage> {
  ...
  Future<void> _saveVideoTape() async {
    ...
    try {
      ...
      final url = Uri.parse(
          "${ApiConstants.baseUrl}/videotapes${widget.tape?.id != null ? "/${widget.tape!.id}" : ""}");
    ...
    try {
      ...
      if (image != null) {
        ...
        if (response.statusCode == 200) {
          ...
          setState(() => _imageUrls.add(data['url']));
        } else {
  ...
  Future<void> _registerExistingImage(String filename) async {
    try {
      final response = await http.post(
        Uri.parse('${ApiConstants.baseUrl}/register-existing-image'),
        headers: {
          'Content-Type': 'application/json',
      ...
      _showErrorSnackBar('Failed to register image: $e');
    }
  }

  void _registerMultipleImages() async {
    final filenames = ['Solo1.jpg', 'Solo2.jpg'];
    for (var filename in filenames) {
      await _registerExistingImage(filename);
    }
  }
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            const SizedBox(height: AppDimensions.marginLarge),
            const Text('Description', style: AppTextStyles.bodyMedium),
    ...
    return Column(
      ...
      children: [
        Row(
          ...
          children: [
            const Text('Images', style: AppTextStyles.headingSmall),
            _buildRegisterExistingImagesButton(),
          ],
        ),
        const SizedBox(height: AppDimensions.marginMedium),
        Container(
      ...
      decoration: BoxDecoration(
        ...
        image: DecorationImage(
          image: _imageUrls[index].startsWith('data:image')
              ...
              : NetworkImage(getFullImageUrl(_imageUrls[index]))
                  as ImageProvider,
          fit: BoxFit.cover,
        ),
      ),
  ...
  Widget _buildLevelSelection() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Age Rating', style: AppTextStyles.bodyMedium),
        const SizedBox(height: AppDimensions.marginSmall),
        Wrap(
  ...
  Widget _buildRegisterExistingImagesButton() {
    return TextButton(
      onPressed: _registerMultipleImages,
      child: const Text('Register Existing Images'),
    );
  }
}
